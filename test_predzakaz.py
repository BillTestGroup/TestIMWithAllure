...
@allure.title("Покупка телефона клиентом ФЛ по предзаказу")
def test_predzakaz(br):
    HB.login_site(br, login, passw)
    HB.check_cart(br)
    HB.cut_pop_up(br)
    external_id, device_price, monthly_payment, full_price = HB.buy_as_predzakaz(br, test_dude)
    HB.wait_for_order_in_work(br)
    time.sleep(30)
    HB.log_in_wso(br, wso, vix_creds)
    HB.check_wso_order(br, test_dude, external_id, device_price, monthly_payment, full_price)
